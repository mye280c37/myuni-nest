...
export class ConverterController {
    ...
    async getConversion (@Res() response: Response, @Query('korean') korean: number, @Query('english') english: number, @Query('math') math: number, @Query('society') society: number, @Query('science') science: number,  @Query('history') history: number, @Query('select') select: number) {
        try {
